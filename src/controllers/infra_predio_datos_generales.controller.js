...
import {InfraPredioFotos} from '../models/infra_predio_fotos.js'
...
export const leeUniOperan = (req, res) => {
    var consulta = `
...
export const listaFotosPredio= (req, res) => {
    console.log(req.params.id); //.findOne({id:req.params.id})
    return InfraPredioFotos.findAll({
        where: {
            infraPredioId: req.params.predioId
        },
        })
        .then(InfraPredioFotos => res.status(200).send(InfraPredioFotos))
        .catch(error => res.status(400).send(error));
}